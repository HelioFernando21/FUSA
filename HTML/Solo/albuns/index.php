...
$_SESSION["galeria"] = $galeria;
				...
				?>
                    ...
                    <div id="logar">
							...
							if ($_GET["id"] == "")
							{
								echo '<script type="text/javascript"> jAlert("Página não encontrada"); </script>';
							}
							else
							{
...
<div id="conteudo">
			...
			<a href="profile.php">
            	...
            	<?php echo '<a href="../profile/profile.php?id='.$_GET["id"].'"><img id="foto_user" src="../../Usuarios/'.$destinoFotoPerfil.'" /></a>';    ?>
            </a>
...
$sql = mysql_query("Select cd_foto,nm_destino_foto from fotos where (cd_usuario = ".$galeria.") and (cd_album = ".$album[$z].");") or die;
	...
	$photo = "photo".$album[$z];
								 ...
								 if ($imagem != "")
								 {
									...
									while($linha = mysql_fetch_array($sql))
									{
									 ...
									 else
									 {
										...
										echo ' <script type="text/javascript">window.location = "index.php?id='.$_GET["id"].'"</script>';
									 }
		...
		while($linha = mysql_fetch_array($sql)){	
			...
			if ($_SESSION["codigo"] == $galeria)
			{
				...
				$qt_fotos++;
			echo '<a href="../../Usuarios/'.$linha[1].'" rel="lightbox[grupo'.$g.']" >
			<div id="foto'.$linha[0].'" name="foto'.$linha[0].'" style="background-image:url(../../Usuarios/'.$linha[1].');" class="img" >
			</div>
			</a>
			</div>';
			}
			...
			<a href="../../Usuarios/'.$linha[1].'" rel="lightbox[grupo'.$g.']" >
			<div id="foto'.$linha[0].'" name="foto'.$linha[0].'" style="background-image:url(../../Usuarios/'.$linha[1].');" class="img" >
			</div>
			</a>
			';
	...
	echo'<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			$("#listas'.$listas.'").css("width","'.$qt_fotos.'px");
